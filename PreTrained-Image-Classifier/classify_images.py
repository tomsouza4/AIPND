...
def classify_images(images_dir, results_dic, model):
    ...
    for key, value in results_dic.items():
        var1 = str(value).strip("[]")
        var2 = var1.strip("'")
        pet_labels.append(var2)
    classifier_labels = []

# If first time key is assigned initialize the list with pet&classifier labels
    for idx in range (0, len(results_dic), 1):
        full_path = images_dir+filenames[idx]
        classifier_labels.append(classifier(full_path, model))

        if filenames[idx] in results_dic.keys():
