"""
Question: Create a function that opens the flowers.txt, reads every line in it,
...
letter of the first name. It should then use it to print the flower name with
...
flowers_dic = {}
message = "Unique flower name with the first letter: {}"

# HINT: create a dictionary from flowers.txt
def create_flowers_list(filename):
    with open(filename, 'r') as flowers:
        for value in flowers:
            flowers_dic[value.split(": ")[0].lower()] = value.strip().split(": ")[1]
    return flowers_dic

# HINT: create a function to ask for user's first and last name
def ask_name():
    name = input("Enter your First [space] Last name only:")[:1].lower()
    return name
# print the desired output
create_flowers_list('flowers.txt')

fletter = ask_name()
if fletter in flowers_dic.keys():
    print(message.format(flowers_dic[fletter]))
