...
class Gaussian():
    ...
    def calculate_stdev(self, sample=True):
        ...
        """
        mean
        deviation from the mean (n-mean)
        SS - sum of squares
        Variance Population / Variance Sample
        Std Deviation sqrt(variance)
        """
        x = self.mean
        ss = sum((i-x)**2 for i in self.data)
        
        if sample:
            variance_sample = ss / (len(self.data)-1)
            self.stdev = math.sqrt(variance_sample)
        else:
            variance_pop = ss / len(self.data)
            self.stdev = math.sqrt(variance_pop)            
